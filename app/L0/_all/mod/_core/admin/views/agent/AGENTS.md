...
- the subscription status summary must not collapse bridge or auth failures into a fake `Codex desktop is not installed on this machine.` message once the local Codex binary has already been detected; surface the real bridge error instead
- the Subscription tab should use the documented Codex `chatgptDeviceCode` login flow: `Sign In` opens the verification page, the dialog shows the returned user code, and the user completes sign-in outside the app before pressing `Refresh`
...
- the Subscription section should also expose a dedicated thinking-effort selector, persisted with the chosen subscription model and passed to the Codex bridge only for Subscription-mode sends; `minimal` must not be offered because the current Codex runtime rejects that effort when tool capability is present, so legacy `minimal` values should normalize to `low`
