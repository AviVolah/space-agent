...
function resolveOnscreenAgentExamplePromptBlock(store) {
  if (store.shouldShowApiKeyWarning) {
    return {
      noticeText: store.subscriptionSetupNoticeText,
...
function createCodexReasoningEffortOptions() {
  return [
    {
      label: "Model default",
      value: config.ONSCREEN_AGENT_CODEX_REASONING_EFFORT.DEFAULT
    },
    {
      label: "Low",
      value: config.ONSCREEN_AGENT_CODEX_REASONING_EFFORT.LOW
    },
    {
      label: "Medium",
      value: config.ONSCREEN_AGENT_CODEX_REASONING_EFFORT.MEDIUM
    },
    {
      label: "High",
      value: config.ONSCREEN_AGENT_CODEX_REASONING_EFFORT.HIGH
    }
  ];
}
...
function summarizeOnscreenAgentLlmSelection(settings, huggingfaceState, codexState) {
...
const model = {
  ...
  settings: {
    ...
    subscriptionReasoningEffort: config.DEFAULT_ONSCREEN_AGENT_SETTINGS.subscriptionReasoningEffort
  },
  settingsDraft: {
    apiEndpoint: "",
    apiKey: "",
  ...
  get shouldShowApiKeyWarning() {
    ...
    if (provider === config.ONSCREEN_AGENT_LLM_PROVIDER.SUBSCRIPTION) {
      return this.isInitialized && !this.isLoadingDefaultSystemPrompt && !this.isConfiguredSubscriptionReady(this.settings);
    }
  ...
  get llmSummary() {
    return summarizeOnscreenAgentLlmSelection(this.settings, this.huggingface, this.codex);
  },
  ...
  get isSettingsDraftUsingSubscriptionProvider() {
    return config.normalizeOnscreenAgentLlmProvider(this.settingsDraft.provider) === config.ONSCREEN_AGENT_LLM_PROVIDER.SUBSCRIPTION;
  },
  ...
  get subscriptionSetupButtonLabel() {
    return config.normalizeOnscreenAgentLlmProvider(this.settings.provider) === config.ONSCREEN_AGENT_LLM_PROVIDER.SUBSCRIPTION
      ? "Connect ChatGPT"
      : "Set LLM API key";
  },

  get subscriptionSetupNoticeText() {
    return config.normalizeOnscreenAgentLlmProvider(this.settings.provider) === config.ONSCREEN_AGENT_LLM_PROVIDER.SUBSCRIPTION
      ? "Connect ChatGPT in model settings before using the overlay."
      : "Don't forget to configure your LLM first.";
  },
  ...
  setDefaultCodexDraftModel() {
    ...
    this.settingsDraft = {
      ...
      subscriptionReasoningEffort:
        config.normalizeOnscreenAgentCodexReasoningEffort(this.settingsDraft.subscriptionReasoningEffort) ||
        config.normalizeOnscreenAgentCodexReasoningEffort(this.codexModels[0]?.defaultReasoningEffort)
    };
    return true;
  },
  ...
  async refreshCodexStatus(options = {}) {
    ...
    try {
      ...
      this.codex = {
        ...this.codex,
        error: getOnscreenAgentErrorMessage(error, "Unable to check ChatGPT subscription status."),
        isLoading: false
      };
  ...
  async startCodexLogin() {
    try {
      ...
      this.codex = {
        ...this.codex,
        loginError: getOnscreenAgentErrorMessage(error, "Unable to start ChatGPT sign-in."),
        loginPending: false
      };
  ...
  async init() {
    ...
    this.initializationPromise = (async () => {
      ...
      try {
        ...
        if (config.normalizeOnscreenAgentLlmProvider(this.settings.provider) === config.ONSCREEN_AGENT_LLM_PROVIDER.SUBSCRIPTION) {
  ...
  async saveSettingsFromDialog() {
    ...
    this.settings = {
      ...
      provider,
      subscriptionReasoningEffort: config.normalizeOnscreenAgentCodexReasoningEffort(
    ...
    try {
      ...
      this.status =
        provider === config.ONSCREEN_AGENT_LLM_PROVIDER.LOCAL
          ...
          : provider === config.ONSCREEN_AGENT_LLM_PROVIDER.SUBSCRIPTION
  ...
  async streamAssistantResponse(requestMessages, assistantMessage, preparedRequest) {
    ...
    const usingSubscriptionProvider =
      config.normalizeOnscreenAgentLlmProvider(this.settings.provider) === config.ONSCREEN_AGENT_LLM_PROVIDER.SUBSCRIPTION;
