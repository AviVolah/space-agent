...
- the Subscription status copy must not fall back to `Codex desktop is not installed on this machine.` after the local Codex binary was already detected; surface the real bridge error so the user can act on it
- the Subscription tab should use the documented Codex `chatgptDeviceCode` login flow: `Sign In` opens the verification page, the dialog shows the returned user code, and the user completes sign-in outside the overlay before pressing `Refresh`
- the Subscription `Sign In` action should stay available whenever the bridge is idle, even if status detection has not yet confirmed installation, so the device-code flow can probe the local bridge directly instead of being blocked by a possibly stale `installed` flag
- Subscription status refresh and sign-in should fail fast on local bridge hangs so the dialog returns to an interactive state instead of leaving both action buttons disabled forever
- the Subscription settings should include a dedicated thinking-effort selector, persisted with the chosen subscription model and sent only on Subscription-mode Codex requests; `minimal` must not be offered because the current Codex runtime rejects that effort when tool capability is present, so legacy `minimal` values should normalize to `low`
...
- the settings modal keeps a provider switch with exactly three tabs named `API`, `Local`, and `Subscription`; API settings show endpoint, model, and API key fields, local settings mount the shared `_core/huggingface/config-sidebar.html` component in `onscreen` mode, and Subscription shows local Codex desktop status plus a subscription model picker and thinking-effort selector
...
- subscription-provider sends should reuse that same prepared prompt payload and folded transport message list, but route the final request through the local `/api/codex_completion` bridge together with the assembled system prompt so Space Agent still owns prompt shaping and Codex acts only as the subscribed transport
...
- the default composer blocker should show `Set LLM API key` only for the API provider when the shipped defaults are still selected and no API key is configured; subscription mode reuses that blocker surface with a `Connect ChatGPT` action whenever the saved Codex subscription setup is not currently ready, while local Hugging Face mode does not require the blocker
