...
- `server/lib/utils/codex_app_server.js`: canonical local Codex desktop bridge for ChatGPT-subscription status, login, model discovery, and ephemeral completion streaming; `codex app-server` reachability is the real runtime signal, while `codex --version` remains best-effort metadata only, startup must send JSON-RPC `initialize` directly without re-entering the public `call()` path, Windows launch should prefer the resolved local Codex binary path rather than assuming a bare PATH-only `codex` command, and stream routing must honor both top-level and nested turn identifiers from Codex notifications
...
- local Codex subscription bridge: `codex_status`, `codex_login_start`, `codex_completion`
