...
- `codex_login_start` is a `POST` endpoint that starts the documented Codex `chatgptDeviceCode` login flow and returns the verification URL, user code, and login id so the frontend can own the sign-in UX without depending on the local callback browser flow
